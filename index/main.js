...
import {
  collection, doc, getDoc, setDoc, onSnapshot, increment, updateDoc, serverTimestamp
...
function updateStatusBar() {
  const sb = $('statusBar');
  if (!currentUser) { sb.className='status-bar info'; sb.textContent='投票するにはログインが必要です'; return; }
  if (myVoteCandId) {
...
function renderCandList() {
  ...
  if (!s.length) {
    $('candList').innerHTML = '<div style="text-align:center;padding:40px;color:var(--muted);font-size:13px">まだ候補者が登録されていません</div>';
    return;
  }

  $('candList').innerHTML = s.map((c,i) => {
    const isMyVote = myVoteCandId === c.id;
    let btnClass='vb-idle', btnLabel='投票する', btnDis='';
    if (isMyVote)       { btnClass='vb-done';   btnLabel='✓ 投票済み'; btnDis='disabled'; }
    else if (hasVoted)  { btnClass='vb-locked'; btnLabel='投票済み';    btnDis='disabled'; }
    else if (!hasUser)  { btnClass='vb-locked'; btnLabel='要ログイン';  btnDis='disabled'; }
...
window.openProfile = function(id) {
  ...
  const hasVoted = !!myVoteCandId;
  const hasUser  = !!currentUser;
  const tags = (c.tags||[]).map(t=>`<span class="badge badge-blue">${t}</span>`).join('');

  let vBtnLabel='🗳️ この候補者に投票する', vBtnDis='';
  if (isMyVote)      { vBtnLabel='✓ この候補者に投票済み'; vBtnDis='disabled'; }
  else if (hasVoted) { vBtnLabel='他の候補者に投票済み';   vBtnDis='disabled'; }
  else if (!hasUser) { vBtnLabel='ログインが必要です';      vBtnDis='disabled'; }
...
window.handleProfBg = e => { if(e.target===$('profileOverlay')) closeProfile(); };
window.closeProfile = () => { $('profileOverlay').classList.remove('open'); document.body.style.overflow=''; };

// ===== SELECT & CONFIRM =====
window.selectCand = function(id) {
  if (!currentUser) { showToast('まずGoogleでログインしてください'); return; }
  if (myVoteCandId) { showToast('すでに投票済みです'); return; }
  const c = candidates.find(x=>x.id===id); if(!c) return;
  pendingCandId = id;
...
window.submitVote = async function() {
  ...
  try {
    ...
    if (existing.exists()) {
      ...
      updateStatusBar(); renderCandList();
    ...
    await updateDoc(doc(db, "candidates", pendingCandId), { votes: increment(1) });
